meta:
  ...
  endian: be
doc: |
  The Send List contains metadata for all the messages the Wii needs to
...
seq:
  ...
  - id: mail_flag
    ...
    size: 40
    encoding: utf-8
    doc: Used to strcmp with the flag given by the server. If different it means there is mail to receive.
  - id: entries
    type: entry
    repeat: expr
    repeat-expr: 127
